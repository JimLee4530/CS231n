...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  cache = []
  for i in xrange(T):
    h[:,i,:],cache_sub = rnn_step_forward(x[:,i,:],prev_h,Wx,Wh,b)
    # print next_h.shape
    prev_h = h[:,i,:]
...
def rnn_backward(dh, cache):
  ...
  ##############################################################################
  # N, T, H = dh.shape
  # x, Wx, prev_h, Wh, b, next_h = cache[0]
  # _, D = x.shape
  # dx = np.zeros((N, T, D))
  # dh0 = np.zeros((N,H))
  # dWx = np.zeros((D,H))
  ...
  #     continue
  #   # print i
  #   dx_tmp, dprev_h_tmp, dWx_tmp, dWh_tmp, db_tmp = rnn_step_backward(dh[:,i,:],cache[i])
  #   dh0 += dprev_h_tmp
  #   dWx += dWx_tmp
  #   dWh += dWh_tmp
  #   db += db_tmp
...
def lstm_forward(x, h0, Wx, Wh, b):
  ...
  cache=[]
  for i in xrange(T):
    h[:,i,:],c,cache_tmp = lstm_step_forward(x[:,i,:],prev_h, prev_c, Wx, Wh, b)
    prev_h = h[:,i,:]
    prev_c = c
