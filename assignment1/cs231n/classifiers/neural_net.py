...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    scores -= np.reshape(np.max(scores, axis=1),(N,1)) #(N,C)
    prob = np.exp(scores)/np.sum(np.exp(scores),axis=1,keepdims=True) #(N,C)
    y_trueClass = np.zeros_like(prob)#(N,C)
    ...
    #############################################################################
    dscores = (prob - y_trueClass)/N # (N,C)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      random_c = np.random.choice(num_train, batch_size, replace = False)
      # random_c = np.random.choice(num_train, batch_size)
      X_batch=X[random_c,:]
      X_batch = X_batch
      y_batch=y[random_c]
  ...
  def predict(self, X):
    ...
    ###########################################################################
    h1 = np.maximum(0, np.dot(X, self.params['W1']) + self.params['b1']) #(N,H)
    scores = np.dot(h1,self.params['W2']) +  self.params['b2']#(N,C)
